...
async function getFakerProducts() {
    ...
    const apiURLProductosTest = "http://localhost:8000/api/productos-test";
    $.ajax({
        dataType: "json",
        method: "GET",
        url: apiURLProductosTest,
        success: function(apiData) {
            renderFakerProducts(apiData);
        }
    })
}

async function renderFakerProducts(apiData) {
    ...
    apiData.forEach(product => {
        const html = ejs.render(plantilla, product)
        document.querySelector('#fakerProductos').innerHTML += html
    })
}
